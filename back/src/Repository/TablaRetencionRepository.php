...
class TablaRetencionRepository extends ServiceEntityRepository
{
    public function __construct(ManagerRegistry $registry)
    {
        parent::__construct($registry, TablaRetencion::class);
    }
    ...
    public function FindSpecialGet($em, $page, $query, $order_key, $order_orientation, $items_per_page): array
    {
        ...
        if (isset($isInactiveOnly[1])) {
            $tablaRetencionResults = $this->createQueryBuilder('tr')
                ->leftJoin("tr.estructuraDocumental", "ed")
                ->where('tr.estado_id = :estado_id')
                ->andWhere('tr.estructuraDocumental IS NOT NULL')
        ...
        foreach ($tablaRetencionResults as $tablaRetencionRow) {
            $ruta = implode(" > ", array_reverse(explode("/", mb_strtoupper(EntityUtils::crearRutaEstructuraDocumental($em, "", $tablaRetencionRow->getEstructuraDocumental())))));

            if($tablaRetencionRow->getEstructuraDocumental() == null) {
                $tablaRetencionRow->getEstructuraDocumental();
            }

            $stringToFilter = strtolower($tablaRetencionRow->getEstructuraDocumental()->getDescripcion() . " " . EntityUtils::crearRutaEstructuraDocumental($em, "", $tablaRetencionRow->getEstructuraDocumental()));
            //echo $ruta." / ".$query."\n";
            if (!empty($query)) {
    ...
    protected function agregarElementoTRD($tablaRetencionRow, $ruta)
    {
        ...
        $tablaRetencionDto->setEstructuraDocumentalId($tablaRetencionRow->getEstructuraDocumentalId());
        ...
        if ($tablaRetencionRow->getEstructuraDocumental()->getCodigoDirectorio() != 0)
            $tablaRetencionDto->setCodigoArchivoDocumental($tablaRetencionRow->getEstructuraDocumental()->getCodigoDirectorioPadre() . "-" . $tablaRetencionRow->getEstructuraDocumental()->getCodigoDirectorio());
        else
            $tablaRetencionDto->setCodigoArchivoDocumental($tablaRetencionRow->getEstructuraDocumental()->getCodigoDirectorioPadre());
