 ...
 * Undocumented class
 */
class EstructuraDocumentalVersionByNodeService
{
    private $_em;
     ...
     */
    public function generarEstructuraDocumentalVersion(string $node, $mostrarPeso): EstructuraDocumentalStandard
    {
        ...
        $estructuraDocumentalVersionRoot = $this->em->getRepository(EstructuraDocumentalVersion::class)->getNode($node);
        //realizar una consulta para saber cuales son los hijos de ese nodo
        $estructuraDocumentalVersionChildren = $this->em->getRepository(EstructuraDocumentalVersion::class)
            ->findBy(
                array("codigo_directorio_padre" => $this->node, "estado_id" => "1", "version" => $estructuraDocumentalVersionRoot->getVersion()),
                array('codigo_directorio_padre' => 'ASC', 'codigo_directorio' => 'ASC', 'peso' => 'ASC')
            );
        if (isset($estructuraDocumentalVersionRoot)) {
            //Guardar la información en estructuraDocumentalStandard
            $this->estructuraDocumentalStandard->setId($estructuraDocumentalVersionRoot->getId());
            $this->estructuraDocumentalStandard->setCodigoDirectorioPadre($estructuraDocumentalVersionRoot->getCodigoDirectorioPadre());
            if ($estructuraDocumentalVersionRoot->getCodigoDirectorio() == $_ENV["CODIGO_DIRECTORIO_FONDO"]) {
                $this->estructuraDocumentalStandard->setCodigoDirectorio("");
            } else {
                $this->estructuraDocumentalStandard->setCodigoDirectorio($estructuraDocumentalVersionRoot->getCodigoDirectorio());
            }
            $this->estructuraDocumentalStandard->setDescripcion($estructuraDocumentalVersionRoot->getDescripcion());
            $this->estructuraDocumentalStandard->setIdEstructura($estructuraDocumentalVersionRoot->getIdEstructura());
            if ($node == "") {
            ...
            $cantidad_caracteres = strlen($estructuraDocumentalVersionRoot->getCodigoDirectorio());
            //Si es un tipo documental se verifica que su código de directorio sea 0
            if ($estructuraDocumentalVersionRoot->getCodigoDirectorio() === 0) {
                $this->estructuraDocumentalStandard->setIcon($_ENV["TIPO_DOCUMENTAL_ICON"]);
            } else if ($cantidad_caracteres == 3 && $estructuraDocumentalVersionRoot->getCodigoDirectorioPadre() == "") {
            ...
            foreach ($estructuraDocumentalVersionChildren as $childNode) {
                $hasTrd = null;
                $trdId = null;
                $generaTrd = false;
                $haveChildrens = false;
                ...
                $childrensNodes = $this->em->getRepository(EstructuraDocumentalVersion::class)
                ...
                if (($childNode->getCodigoDirectorio() !== "0" && $childNode->getType() != "tipo_documental")) {
                    $hijosActivos = $this->em->getRepository(EstructuraDocumentalVersion::class)
                        ->checkActiveChildNodes($childNode->getCodigoDirectorio(), $childNode->getVersion());
                }
                ...
                if ($childNode->getCodigoDirectorio() === "0" || (strlen($childNode->getCodigoDirectorio()) >= $serie && strlen($childNode->getCodigoDirectorio()) <= $subserie)) {
                    $trd = $this->em->getRepository(TablaRetencionVersion::class)
                        ->findOneBy(array("estructura_documental_version_id" => $childNode->getId()));
                    if (isset($trd)) {
                        $hasTrd = true;
                        $trdId = $trd->getId();
                    } else if (!isset($trd)) {
