...
class EstructuraDocumentalXlsService
{
    private $_em;
    ...
    public function get($nodo)
    {
        ...
        $xls = $this->generarExcel($arbolFinal, $nombreFondo, $version);
    ...
    public function getNombreFondo()
    {
        ...
        $entityResults = $queryBuilder
            ->select("estructura.descripcion, estructura.version")
            ->from('App\\Entity\\EstructuraDocumental', 'estructura')
    ...
    public function getChildrens($parent)
    {
        $queryBuilder = $this->em->createQuery("select estructura.codigo_directorio,
                                                        ...
                                                        estructura.type
                                                    from App\\Entity\\EstructuraDocumental estructura
                                                    left join estructura.tablaRetencion tablar
                                                    where estructura.codigo_directorio_padre = '{$parent}' AND estructura.estado_id=1 order by estructura.codigo_directorio asc, estructura.peso asc");
    ...
    public function getNodoPrincipal($parent)
    {
        $queryBuilder = $this->em->createQuery("select estructura.codigo_directorio,
                                                    ...
                                                    from App\\Entity\\EstructuraDocumental estructura
                                                    left join estructura.tablaRetencion tablar
                                                    where estructura.codigo_directorio = '{$parent}'  AND estructura.estado_id=1 order by estructura.codigo_directorio asc");
        $iterableResult = $queryBuilder->iterate();
    ...
    public function generarExcel($arbol, $nombreFondo, $version)
    {
        ...
        foreach ($arbol as $nodo) {
            if ($nodo['codigo_directorio_padre'] == $_ENV["CODIGO_DIRECTORIO_FONDO"] || strlen($nodo['codigo_directorio']) == ($_ENV['SECCION'] + $_ENV['SUBSECCION'])) {
                ...
                $this->nuevaHoja($hoja, $nodo['descripcion'], $nodo['codigo_directorio'], $nombreFondo, $version);

                $proceso=$nodo['descripcion'];
            ...
            //Setear código SGI
            $formulario = $this->em->getRepository(Formulario::class)->findOneBy(array("estructura_documental_id" => $nodo['id']));
            if (null !== $formulario && ($formulario->getNomenclaturaFormulario() != "N/A")) {
                $nodo['codigo'] = $formulario->getNomenclaturaFormulario() . " " . $formulario->getFechaVersion()->format("Y-m-d") . " " . $formulario->getVersion();
            }
    ...
    public function getTitulos()
    {
        ...
        $titulos = array(
            'CÓDIGO', 'SERIES, SUBSERIES  Y TIPOS DOCUMENTALES', '', '', '', '', 'CÓDIGO SGI',
    ...
    public function nuevaHoja($index, $titulo, $codigo, $nombreFondo, $version)
    {
        ...
        $objWorkSheet->setCellValue('U2', '01/08/2018');
        $objWorkSheet->setCellValue('U3', 'V  ' . $version);
